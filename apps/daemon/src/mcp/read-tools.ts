// Read-only MCP tools that proxy to the local daemon HTTP API.
// The 7 tools (list_projects, get_active_context, get_project, list_files,
// get_file, get_artifact, search_files) plus their helpers, moved verbatim
// from the original apps/daemon/src/mcp.ts. See docs/internal/mcp-write-design.md §8.
...
export function registerReadTools(registry: McpToolRegistry, ctx: McpContext): void {
  const { baseUrl } = ctx;
  const handler: McpToolHandler = async (name, rawArgs) => {
    const args = (rawArgs ?? {}) as McpArgs;
    switch (name) {
      case 'list_projects':
...
// Resource handlers (od://focus/active, od://skills/*, od://design-systems/*)
// are wired here because they fetch from the same daemon HTTP API
// surface as the read tools and share helpers (getJson, oneLine).
export async function listReadResources(baseUrl: string) {
...
export async function readReadResource(baseUrl: string, uri: string | undefined) {
